...
public class ManhuntCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("tracker")) {
            switch (args[1].toLowerCase(Locale.ROOT)) {
                case "positional":
                    gameManager.getTrackerManager().setTrackerState(TrackerState.POSITIONAL);
                    sender.sendMessage(prefix + ChatColor.GOLD + "Tracker set to positional.");
                    break;
                case "vector":
                    gameManager.getTrackerManager().setTrackerState(TrackerState.VECTOR);
                    sender.sendMessage(prefix + ChatColor.GOLD + "Tracker set to vector.");
                    break;
                case "nearby":
                    gameManager.getTrackerManager().setTrackerState(TrackerState.NEARBY);
                    sender.sendMessage(prefix + ChatColor.GOLD + "Tracker set to nearest hunter.");
                    break;
                default:
                    sender.sendMessage(prefix + ChatColor.RED + "Tracker mode not found.");
                    break;
            }
            return true;
        }
            ...
            if (args[0].equalsIgnoreCase("distance")) {
            ...
            } else if (args[0].equalsIgnoreCase("reload") && args[1].equalsIgnoreCase("world")) {
                if (!(sender.hasPermission("manhunt.reload.world"))) {
                    sender.sendMessage(ChatColor.translateAlternateColorCodes('&', Objects.requireNonNull(gameManager.getPlugin().getConfig().getString("permission-message"))));
                    return true;
                }
                if (gameManager.getGameState() == GameState.ACTIVE) {
                ...
                sender.sendMessage(prefix + ChatColor.GOLD + "Starting world reload");
                gameManager.getWorldGeneration().createNewGameWorld(sender);
                sender.sendMessage(prefix + ChatColor.GOLD + "World reload finished");
                return true;
            ...
            if ((args[0].equalsIgnoreCase(addHunter) || args[0].equalsIgnoreCase(addRunner)) && Bukkit.getPlayerExact(newPlayer) == null) {
                ...
                Bukkit.broadcastMessage(prefix + ChatColor.AQUA + newPlayer + " is now a Runner");
    ...
    private void messageInfo(CommandSender sender) {
        sender.sendMessage(ChatColor.GOLD + " --- Manhunt V" + gameManager.getPlugin().getDescription().getVersion() + " Made By: " +
                ChatColor.LIGHT_PURPLE + gameManager.getPlugin().getDescription().getAuthors() + ChatColor.GOLD + " --- ");
        ...
        sender.sendMessage(ChatColor.AQUA + "/Manhunt Hadd     " + ChatColor.GRAY + "  Add player to Hunter");
        sender.sendMessage(ChatColor.AQUA + "/Manhunt Radd     " + ChatColor.GRAY + "  Add player to Runner");
        sender.sendMessage(ChatColor.AQUA + "/Manhunt Start    " + ChatColor.GRAY + "  Starts the game");
        sender.sendMessage(ChatColor.AQUA + "/Manhunt Stop     " + ChatColor.GRAY + "  Stops the game");
        sender.sendMessage(ChatColor.AQUA + "/Manhunt Reload   " + ChatColor.GRAY + "  Reload the plugin");
    }
